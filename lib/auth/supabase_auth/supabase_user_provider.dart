...
class BibliotecaSupabaseUser extends BaseAuthUser {
  BibliotecaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> bibliotecaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BibliotecaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
